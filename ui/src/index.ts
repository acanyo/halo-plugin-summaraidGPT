            ...
            },
            {
              ...
              action: async (item?: ListedPost) => {
                ...
                try {
                  ...
                  const tags: string[] = Array.isArray(generateData?.tags) ? generateData.tags : [];
                  if (!generateData?.success || tags.length === 0) {
                    Toast.error(generateData?.message || '未能生成有效标签');
                    return;
                  }
                  ...
                  Dialog.warning({
                    title: 'AI 推荐标签',
                    description: `生成的标签：${tags.join('，')}\n\n是否应用这些标签到文章？`,
                    onConfirm: async () => {
                      try {
                        // 应用标签到文章
                        const { data: applyData } = await axios.post(
                          `/apis/api.summary.summaraidgpt.lik.cc/v1alpha1/createAndUpdateTags`,
                          { postName, tags },
                          { headers: { 'Content-Type': 'application/json' } }
                        );

                        if (applyData?.success) {
                          Toast.success(`成功应用 ${tags.length} 个标签到文章`);
                        } else {
                          Toast.error(applyData?.message || '应用标签失败');
                        }
                      } catch (applyError) {
                        if (applyError instanceof AxiosError) {
                          Toast.error(applyError.response?.data?.message || '应用标签失败');
                        } else {
                          Toast.error('应用标签异常');
                        }
                      }
